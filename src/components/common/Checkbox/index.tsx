...
const Checkbox: React.FC<CheckboxProps> = ({ label, className, disabled, style, ...props }) => {
  const id = useRef(shortid.generate());

  return (
    <View isRow className={cn('cmp-checkbox', { 'cmp-checkbox__disabled': disabled }, className)} style={style}>
      <input
        id={id.current}
        type="checkbox"
      ...
      <View isRow>
        {options?.map((option, index) => (
          <Checkbox
            ...
            style={customColumnMargin ? { marginRight: customColumnMargin + 'px' } : { width: `${100 / columns}%` }}
            disabled={disabled}
            name={name}
          />
        ))}
